	...
	if Dist <= CH_Armory_Locker.Config.DistanceToHeader then 
		...
		cam.Start3D2D( pos, ang, 0.035 )
			if CH_Armory_Locker.Config.DisplayHeaderText then
				draw.SimpleTextOutlined( _RDS.Config.WeaponchestText, "ARMORY_OverheadTitle", 0, -0, CH_Armory_Locker.Design.ArmoryHeaderColor, 1, 1, 4, CH_Armory_Locker.Design.ArmoryHeaderBoarder )
			end
		cam.End3D2D()
	end
