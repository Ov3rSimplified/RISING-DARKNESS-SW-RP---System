net.Receive( "ARM_LOCKER_StealItem", function(length, ply)
	...
	local locker = CH_Armory_Locker.PoliceLockerEntity
	...
	-- bLogs support
	hook.Run( "CH_PoliceArmoryRobbery_bLogs_StealItem", ply, TheItem.Name )
end )

net.Receive( "ARM_LOCKER_FinishLockpicking", function( length, ply )
	local locker = net.ReadEntity()
	local locker_num = net.ReadUInt( 8 )
	...
	if locker then
		...
		local locker_bodygroup = locker_num + 1
		
		locker:SetBodygroup( locker_bodygroup, 1 ) -- "empty" the locker bodygroup
		
		timer.Simple( CH_Armory_Locker.Config.DoorCooldown, function()
			if IsValid( locker ) then
				locker:SetBodygroup( locker_bodygroup, 0 ) -- "restock" the weapons inside bodygroup
		...
		if CH_Armory_Locker.Config.EXP2SystemEnabled then
			EliteXP.CheckXP( ply, CH_Armory_Locker.Config.XPSuccessfulLockpick )
		end

		-- Give XP (DarkRP essentials & Brick's Essentials)
		if CH_Armory_Locker.Config.EssentialsXPSystemEnabled then
			ply:AddExperience( CH_Armory_Locker.Config.XPSuccessfulLockpick, "for successfully lockpicking the police armory." )
		end

		-- Give XP (GlorifiedLeveling)
		if CH_Armory_Locker.Config.GlorifiedLevelingXPSystemEnabled then
			GlorifiedLeveling.AddPlayerXP( ply, CH_Armory_Locker.Config.XPSuccessfulLockpick )
		end
		
		-- bLogs support
		hook.Run( "CH_PoliceArmoryRobbery_bLogs_Robbery", ply )
	end
end )
