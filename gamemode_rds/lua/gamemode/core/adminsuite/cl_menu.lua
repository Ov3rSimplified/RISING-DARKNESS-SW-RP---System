...
function RDSUI:RegisterSelection()
    ...
    function sbar.btnDown:Paint(w, h) return end
        RDSUI.Scroll = Scroll
    _RDS.Adminsuite.UI.Select:AddSheet( "Arabica", Panel, Color(0,255,157), "materials/rdsrp/icon/admin_setting.png")
    RDSUI:Selection()
end
...
function RDSUI:ServerSettings()
    ...
    for k,v in SortedPairsByMemberValue(_RDS.Adminsuite.UI.ServerConfigModules, "id", false) do
        local DCollapsible = vgui.Create( "DCollapsibleCategory", RDSUI.Scroll )	-- Create a collapsible category
        DCollapsible:SetLabel( "" )					
        DCollapsible:Dock( TOP )		
        DCollapsible:DockMargin(5,5,5,0) 
        DCollapsible:SetHeaderHeight(scrh*0.09)
            DCollapsible.Paint = function(self,w,h)
               ...
               draw.SimpleText(v.name, "RDS.Admin.Title", self:GetWide()/2, scrh*0.040, _RDS:Color("WHITE", 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            end
        local panel = vgui.Create("DPanel", DCollapsible)
        panel:Dock(TOP)
        panel:SetTall(scrh*0.3)
        panel.Paint = function(self,w,h)
            draw.RoundedBoxEx(14,0,0,w,h,Color(46,46,46),false,false,true,true)
            draw.RoundedBoxEx(14,2,0,w-4,h-4,Color(59,59,59),false,false,true,true)
        end
        local button = panel:Add("DButton")
        button:Dock(RIGHT)
        button:DockMargin(0,120,40,120)
        button:SetWide(scrh*0.25)
            button.Paint = function(self,w,h)
