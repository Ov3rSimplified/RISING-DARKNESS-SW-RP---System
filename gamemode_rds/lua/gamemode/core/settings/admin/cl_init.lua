...
local function Open()  
    ...
    Gtbl.Main = vgui.Create("DScrollPanel", _RDS.PlayerSettings.Frame)
    Gtbl.Main:Dock(FILL)
    ...
    _RDS.PlayerSettings.FILL:AddSheet( "Arabica", Gtbl.Main, Color(255,0,0), "materials/rdsrp/icon/admin_setting.png")
end 
...
function Gtbl.Player()
    ...
    local function charsettings()
            ...
            charactersettings:SetTall( scrh * 0.6 )
            ...
            topp:SetText( "" )
                ...
                topp.Paint = function(self,w,h)
                    ...
                    draw.SimpleText("Character["..tostring(LocalPlayer().CharacterID).."] - "..v:GetName(), "RDS.Admin.btn", self:GetWide()/2, self:GetTall()/2, clr, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                end
                ...
                can_top:SetText( "" )
                    ...
                    for k,v in pairs(_RDS.Advtraining.Trainings) do 
                      ...
                      if _RDS.Advtraining.Whitelist[k][LocalPlayer().CharacterID] then  
                            ...
                            if v.restrictions[ LocalPlayer():Team() ] then 

                            local item = vgui.Create("DButton", can)
                            item:Dock(TOP)
                            ...
                            item:SetText("")
                                ...
                                item.DoClick = function()
                                    ...
                                    Menu:AddOption( "Geben.", function()
                                        ...
                                        local merge = { [LocalPlayer().CharacterID] = true }
            ...
            have:SetWide( scrw * 0.332)
                ...
                can_have:SetText( "" )
                    ...
                    for k,v in pairs(_RDS.Advtraining.Trainings) do
                        if not _RDS.Advtraining.Whitelist[k][LocalPlayer().CharacterID] then  
                            continue
                        end

                        local item = vgui.Create("DButton", have)
                        item:Dock(TOP)
                        item:DockMargin(10,6,10,0)
                        item:SetTall( scrh * 0.1 )
                        item:SetText("")
                            ...
                            item.DoClick = function()
                                ...
                                Menu:AddOption( "Entfernen.", function()
                                    net.Start("RDSRP.Roleplay.Trainings.Load")
                                    _RDS.Advtraining.Whitelist[k][ LocalPlayer().CharacterID ] = nil
                                        _RDS.WriteCTable(_RDS.Advtraining.Whitelist) 
                                    net.SendToServer()
...
net.Receive("RDSRP.Roleplay.Trainings.ReLoad", function(len)
    ...
    _RDS.Advtraining.Whitelist = read 
