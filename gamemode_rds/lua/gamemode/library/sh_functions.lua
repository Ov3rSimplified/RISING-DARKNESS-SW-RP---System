...
local function Load()
    ...
    _RDS.Shadow.RenderTarget = GetRenderTarget("_RDS.Shadow_original_" .. resStr, ScrW(), ScrH())

    _RDS.Shadow.RenderTarget2 = GetRenderTarget("_RDS.Shadow_shadow_" .. resStr, ScrW(), ScrH())

    _RDS.Shadow.ShadowMaterial = CreateMaterial("_RDS.Shadow", "UnlitGeneric", {
        ["$translucent"] = 1,
        ["$vertexalpha"] = 1,
        ["alpha"] = 1
    })

    _RDS.Shadow.ShadowMaterialGrayscale = CreateMaterial("_RDS.Shadow_grayscale", "UnlitGeneric", {
        ["$translucent"] = 1,
        ["$vertexalpha"] = 1,
        ["$alpha"] = 1,
        ["$color"] = "255 181 18",
    ...
    _RDS.Shadow.BeginShadow = function()

        render.PushRenderTarget(_RDS.Shadow.RenderTarget)
  ...
  _RDS.Shadow.EndShadow = function(intensity, spread, blur, opacity, direction, distance, _shadowOnly)
    ...
    render.CopyRenderTargetToTexture(_RDS.Shadow.RenderTarget2)

    if blur > 0 then
      render.OverrideAlphaWriteEnable(true, true)
      render.BlurRenderTarget(_RDS.Shadow.RenderTarget2, spread, spread, blur)
      render.OverrideAlphaWriteEnable(false, false)
    end

    render.PopRenderTarget()

    _RDS.Shadow.ShadowMaterial:SetTexture('$basetexture', _RDS.Shadow.RenderTarget)
    _RDS.Shadow.ShadowMaterialGrayscale:SetTexture('$basetexture', _RDS.Shadow.RenderTarget2)

    local xOffset = math.sin(math.rad(direction)) * distance
        local yOffset = math.cos(math.rad(direction)) * distance

        _RDS.Shadow.ShadowMaterialGrayscale:SetFloat("$alpha", opacity / 255)
        render.SetMaterial(_RDS.Shadow.ShadowMaterialGrayscale)
        ...
        if not _shadowOnly then
            _RDS.Shadow.ShadowMaterial:SetTexture('$basetexture', _RDS.Shadow.RenderTarget)
            render.SetMaterial(_RDS.Shadow.ShadowMaterial)
  ...
  _RDS.Shadow.DrawShadowTexture = function(texture, intensity, spread, blur, opacity, direction, distance, shadowOnly)
    ...
    render.CopyTexture(texture, _RDS.Shadow.RenderTarget2)

    if blur > 0 then
        render.PushRenderTarget(_RDS.Shadow.RenderTarget2)
        render.OverrideAlphaWriteEnable(true, true)
        render.BlurRenderTarget(_RDS.Shadow.RenderTarget2, spread, spread, blur)
        render.OverrideAlphaWriteEnable(false, false)
        render.PopRenderTarget()
    ...
    if not shadowOnly then
        ...
        _RDS.Shadow.ShadowMaterial:SetTexture('$basetexture', texture)
        render.SetMaterial(_RDS.Shadow.ShadowMaterial)
        render.DrawScreenQuad()
        end
    end
