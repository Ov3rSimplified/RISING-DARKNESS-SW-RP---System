...
--print(_RDS.Admin.IConfig["Weaponsystem"].Core.MaxSecondary)
...
net.Receive( "RDSRP.Weaponchest.RetrieveItem", function( length, ply )
	...
	if tablee.categorys.category == "Primary" then 
		if ply.PrimaryWeapons + 1 > _RDS.Admin.IConfig["Weaponsystem"].Core.MaxPrimary then 
	...
	if tablee.categorys.category == "Secondary" then 
		if ply.SecondaryWeapons + 1 > _RDS.Admin.IConfig["Weaponsystem"].Core.MaxSecondary then 
