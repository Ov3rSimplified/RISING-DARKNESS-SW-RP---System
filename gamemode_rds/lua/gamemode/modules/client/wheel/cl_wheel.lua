...
local clrWhite = _RDS.PlayerConfig.config["UIBaseColor"]
...
function MVPPerfectHands.ShowRadialMenu(sections)
    ...
    if IsValid(MVPPerfectHands.panel) then
        MVPPerfectHands.panel:Remove() 
    end

    MVPPerfectHands.panel = vgui.Create('DButton')
    MVPPerfectHands.panel:SetSize(w(), h())
    MVPPerfectHands.panel:Center()
    MVPPerfectHands.panel:MakePopup()
    MVPPerfectHands.panel:SetCursor('hand')
    MVPPerfectHands.panel.selectedArea = 0
    MVPPerfectHands.panel:SetText('')
    MVPPerfectHands.panel.selectedText = ''

    function MVPPerfectHands.panel:Think()
        if not sections[self.selectedArea + 1] then return end
        self.selectedOption = sections[self.selectedArea + 1]
    end

    function MVPPerfectHands.panel:DrawMenu(w, h)
    ...
    function MVPPerfectHands.panel:Paint(w, h)
        MVPPerfectHands.panel:DrawMenu(w, h)

        draw.SimpleText(self.selectedOption.title, 'mvp.Wheelmenu.Title', origin_w, origin_h + 35 - offsetIconGroup, txtcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        
        if self.selectedOption.description then
            draw.SimpleText(self.selectedOption.description, 'mvp.Wheelmenu.Hint', origin_w, origin_h + 60 - offsetIconGroup, txtcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        ...
        local textLeftClickW, textLeftClickH = draw.SimpleText('LMB to select', 'mvp.Wheelmenu.Hint', origin_w, origin_h + offsetIconGroup * .8, txtcolor, TEXT_ALIGN_CENTER)
        ...
        draw.SimpleText('RMB to close', 'mvp.Wheelmenu.Hint', origin_w, origin_h + offsetIconGroup * 1.4, txtcolor, TEXT_ALIGN_CENTER)
    ...
    function MVPPerfectHands.panel:DoClick()
    ...
    function MVPPerfectHands.panel:DoRightClick()
        surface.PlaySound(soundClick)
        self:Remove()  
    end
end